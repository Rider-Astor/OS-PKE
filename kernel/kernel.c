...
//   sprint("trapfram ok\n");
  ...
  load_bincode_from_host_elf(proc); //from now on , tp is storing the hart_context
...
int s_start(void) {
  ...
  enable_paging();
//   sprint("kernel pagetable addr: %lx\n", g_kernel_pagetable);
//   sprint("kernel page table is on \n");
  ufree_page[read_tp()] = USER_FREE_ADDRESS_START;
  // the application code (elf) is first loaded into memory, and then put into execution
  load_user_program(&user_app[read_tp()]); //TO_DO
